...
| **LLM interaction** | `actors/llms.py` (+ `actors/proxy_openai.py`, `actors/proxy_genai.py`), `chats.py`, `prompting.py` | LLM abstraction — base `LLMChat` (`llms.py`) with the `OpenAI` and `GoogleGenAI` backends in their own modules (re-exported from `llms`), conversation management, schema processing |
...
session), `.devcontainer/Dockerfile.test` provides a headless image with all
...
docker build --network=host -t kbench-dev -f .devcontainer/Dockerfile.test .
...
It shares a base image with the interactive devcontainer
(`.devcontainer/Dockerfile`), so the two pull the same layers.

Expected: the suite passes. The browser-backed tests in `tests/tools/test_web.py`
skip, because Playwright browsers are not installed here — fetching their system
libraries needs apt, and bullseye's security pool no longer serves them.
