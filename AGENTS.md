...
- **Actor system** → `src/kaggle_benchmarks/actors/` (`LLMChat` in `llms.py`, with the `OpenAI` and `GoogleGenAI` backends in `proxy_openai.py`/`proxy_genai.py` re-exported from `llms.py`; concrete `Tool`/`system`/`user` actors in `base.py`; `Actor` base class in `core.py`)
