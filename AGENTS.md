...
### Subsystems
- **Actor system** → `src/kaggle_benchmarks/actors/` (base `LLMChat` and `Actor` in `llms.py`/`base.py`; the `OpenAI` and `GoogleGenAI` backends in `proxy_openai.py`/`proxy_genai.py`, re-exported from `llms.py`)
